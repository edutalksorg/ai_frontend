...
import { Lock, Loader, ArrowLeft, Mic } from 'lucide-react';
...
const PrivacyPage: React.FC = () => {
    ...
    useEffect(() => {
        const fetchContent = async () => {
            try {
                ...
                if (settings.footer_privacy) {
                    try {
                        const parsed = typeof settings.footer_privacy === 'string'
                            ? JSON.parse(settings.footer_privacy)
                            : settings.footer_privacy;
                        setContent(parsed);
                    } catch (e) {
                        setContent(prev => ({ ...prev, description: settings.footer_privacy }));
                    }
                }
            } catch (error) {
    ...
    return (
        <div className="min-h-screen bg-slate-950 text-slate-200 flex flex-col p-6 relative overflow-x-hidden">
            ...
            <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-emerald-600/10 blur-[150px] rounded-full -translate-y-1/2 translate-x-1/4" />
            <div className="absolute bottom-0 left-0 w-[500px] h-[500px] bg-teal-600/10 blur-[150px] rounded-full translate-y-1/2 -translate-x-1/4" />
            ...
            <div className="z-10 w-full max-w-5xl mx-auto py-12 md:py-20 animate-in fade-in slide-in-from-bottom-6 duration-1000">
                ...
                <div className="text-left mb-16">
                    <Link to="/" className="inline-flex items-center gap-3 group">
                        <div className="bg-gradient-to-br from-emerald-600 to-teal-600 p-2.5 rounded-2xl group-hover:scale-110 transition-transform shadow-xl shadow-emerald-600/20">
                ...
                <div className="bg-white/5 border border-white/10 backdrop-blur-2xl shadow-2xl p-8 md:p-16 rounded-[2.5rem]">
                    <div className="mb-12 text-left">
                        <div className="inline-flex items-center justify-center p-3 bg-emerald-600/20 rounded-2xl mb-6 backdrop-blur-sm border border-emerald-500/20">
                            <Lock size={32} className="text-emerald-400" />
                        </div>
                        <h1 className="text-4xl md:text-6xl font-black text-white tracking-tight leading-[1.1] mb-6">
                            {content.title}
                        </h1>
                        <div className="h-2 w-32 bg-emerald-600 rounded-full" />
                    </div>
                                ...
                                const renderWithLinks = (text: string) => {
                                    ...
                                    return parts.map((part, i) => {
                                        if (emailRegex.test(part)) {
                                            return (
                                                <a key={i} href={`mailto:${part}`} className="text-emerald-400 hover:text-emerald-300 underline underline-offset-4 decoration-emerald-500/30 transition-colors">
                                                    {part}
                                                </a>
                                            );
                                        }
                ...
                <div className="mt-20">
                    <div className="flex flex-col md:flex-row justify-between items-center bg-white/5 border border-white/5 backdrop-blur-md px-8 py-6 rounded-3xl gap-6">
                        ...
                        <div className="flex flex-wrap justify-center gap-6 text-sm font-semibold">
                            <Link to="/privacy-policy" className="text-emerald-400">
                                Privacy Policy
                            </Link>
