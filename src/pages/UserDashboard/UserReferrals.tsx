...
const UserReferrals: React.FC = () => {
    ...
    const copyToClipboard = () => {
        ...
        dispatch(showToast({ message: t('referrals.linkCopied'), type: 'success' }));
    };

    if (loading) return <div className="text-center py-12 text-slate-500">{t('referrals.loading')}</div>;
    ...
    return (
        <div className="space-y-6 md:space-y-8">
            ...
            <div className="flex items-center gap-3 md:gap-4">
                ...
                <h1 className="text-xl md:text-2xl lg:text-3xl font-bold text-slate-900 dark:text-white">{t('referrals.title')}</h1>
            </div>
            ...
            <div className="bg-gradient-to-r from-blue-500 to-indigo-600 rounded-xl md:rounded-2xl p-6 md:p-8 text-white text-center shadow-lg">
                <h2 className="text-2xl md:text-3xl font-bold mb-2">{t('referrals.subtitle')}</h2>
                <p className="text-sm md:text-base mb-6 md:mb-8 opacity-90">{t('referrals.description')}</p>
                ...
                <div className="max-w-3xl mx-auto flex flex-col gap-6 pt-4">
                    ...
                    <div className="flex flex-col sm:flex-row gap-4 items-end justify-between">
                        <div className="flex-1 min-w-0 text-left w-full space-y-2">
                            <p className="text-xs uppercase tracking-wider opacity-75 text-blue-100 font-semibold pl-1">{t('referrals.yourLink')}</p>
                        ...
                        <Button onClick={() => { navigator.clipboard.writeText(referralLink); dispatch(showToast({ message: t('referrals.linkCopied'), type: 'success' })); }} className="bg-white/20 text-white hover:bg-white/30 border border-white/40 shadow-lg shrink-0 whitespace-nowrap px-8 py-3 rounded-xl h-[46px]">
                    ...
                    <div className="flex flex-col sm:flex-row gap-4 items-end justify-between">
                        <div className="flex-1 min-w-0 text-left w-full space-y-2">
                            <p className="text-xs uppercase tracking-wider opacity-75 text-blue-100 font-semibold pl-1">{t('referrals.yourCode')}</p>
                            <div className="bg-black/20 px-4 py-3 rounded-xl flex items-center border border-blue-300">
                                <p className="font-mono text-xl font-bold tracking-widest text-white w-full">{referralCode}</p>
                            </div>
                        </div>
                        <Button onClick={() => { navigator.clipboard.writeText(referralCode); dispatch(showToast({ message: t('referrals.codeCopied'), type: 'success' })); }} className="bg-white/20 text-white hover:bg-white/30 border border-white/40 shadow-lg shrink-0 whitespace-nowrap px-8 py-3 rounded-xl h-[46px]">
                            <Copy size={18} className="mr-2" /> {t('referrals.copyCode')}
                        </Button>
                    </div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
                <div className="card text-center p-4 md:p-6">
                    ...
                    <p className="text-slate-500">{t('referrals.friendsJoined')}</p>
                </div>
                <div className="card text-center p-6">
                    ...
                    <p className="text-slate-500">{t('referrals.totalEarnings')}</p>
                </div>
                <div className="card text-center p-6">
                    ...
                    <p className="text-slate-500">{t('referrals.yourLevel')}</p>
                </div>
            </div>

            {/* History */}
            <div className="card">
                <h3 className="text-base md:text-lg font-bold mb-4 text-slate-900 dark:text-white">{t('referrals.history')}</h3>
                <div className="overflow-x-auto -mx-4 sm:mx-0">
                    <table className="w-full min-w-[640px] text-left">
                        <thead>
                            <tr className="border-b border-slate-200 dark:border-slate-700 text-sm text-slate-500">
                                <th className="pb-3 px-4">{t('referrals.user')}</th>
                                <th className="pb-3 px-4">{t('referrals.date')}</th>
                                <th className="pb-3 px-4">{t('referrals.status')}</th>
                                <th className="pb-3 px-4 text-right">{t('referrals.reward')}</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
                            {history.length > 0 ? (
                                ...
                                <tr>
                                    <td colSpan={4} className="py-8 text-center text-slate-500 italic">{t('referrals.noReferrals')}</td>
                                </tr>
                            )}
                        </tbody>
                    </table>
