...
import { Check, Star, Shield, Zap, AlertCircle, ArrowLeft, Tag, X, Crown, Sparkles } from 'lucide-react';
...
const UserSubscriptions: React.FC = () => {
    ...
    const processSubscription = async (plan: any) => {
        try {
            ...
            await fetchData();
            dispatch(updateUserSubscription({ subscriptionStatus: 'active', subscriptionPlan: plan.name || 'Pro' }));
        } catch (e: any) {
            const errorMsg = e.response?.data?.messages?.[0] || 'Failed to activate subscription';
            dispatch(showToast({ message: errorMsg, type: 'error' }));
        }
    };
                    ...
                    {plans.length > 0 ? plans.map((plan) => {
                        ...
                        return (
                            <div key={plan.id || plan._id} className={`glass-card relative rounded-3xl p-6 sm:p-8 flex flex-col h-full transform transition-all duration-300 hover:-translate-y-2 hover:shadow-2xl ${isYearlyPlan
                                ...
                                <p className="text-sm text-slate-600 dark:text-slate-400 mb-8 min-h-[40px] leading-relaxed">
                                        ...
                                        Object.entries(plan.features).map(([key, value]: [string, any], i: number) => {
                                            ...
                                            return (
                                                <li key={i} className="flex items-start gap-3 text-sm text-slate-700 dark:text-slate-300">
                                                    <div className={`mt-0.5 p-0.5 rounded-full ${isYearlyPlan ? 'bg-violet-100 dark:bg-violet-900/50 text-violet-600 dark:text-violet-400' : 'bg-green-100 dark:bg-green-900/50 text-green-600 dark:text-green-400'}`}>
                                ...
                                {/* Coupon & Actions */}
                                <div className="mt-auto space-y-4 pt-6 border-t border-slate-200/50 dark:border-white/10">
                                    ...
                                    {(() => {
                                        ...
                                        if (!isLocked) {
                                            if (appliedCoupon) {
                                                ...
                                                return <button onClick={() => toggleCouponInput(planId)} className="text-xs text-blue-500 hover:underline flex items-center gap-1">+ {t('subscriptionsPageView.haveCoupon')}</button>
                                            }
                                        }
                                    })()}
